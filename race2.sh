...
for n in "${NAMES[@]}"; do rm -f "$OUT/r2_$n.log"; done
pkill -9 -f "Xvfb :79" 2>/dev/null; rm -f /tmp/.X79-lock /tmp/.X11-unix/X79
Xvfb :79 -screen 0 2400x700x24 >/dev/null 2>&1 & sleep 2

pkill -f "np64-relay --bind 127.0.0.1:6465" 2>/dev/null
if ! (ss -ulnp 2>/dev/null | grep -q ":6465 "); then
  /work/relay-target/release/np64-relay --bind 127.0.0.1:6465 --verbose >"$OUT/r2_relay.log" 2>&1 &
fi
sleep 1
...
for n in "${NAMES[@]}"; do
  mkdir -p "/tmp/r2_$n"
  HOME="/tmp/r2_$n" NP64_ENABLE=1 NP64_LOG=1 NP64_TRACE_IO=1 NP64_RELAY=127.0.0.1:6465 NP64_ROOM=$ROOM NP64_NAME="$n" \
    "$ARESBIN" "${COMMON[@]}" /work/mk64_test.z64 >"$OUT/r2_$n.log" 2>&1 &
  PIDS+=($!)
  sleep 2
done
...
for i in $(seq 1 "${DWELL:-150}"); do
  ...
  if [ "$i" = "${SHOT1:-90}" ] || [ "$i" = "${SHOT2:-120}" ]; then
    ...
    sleep 1
    scrot -o "$OUT/r2_i${i}.png" 2>/dev/null
  fi
done
...
for n in "${NAMES[@]}"; do
  echo "$n: $(grep -oiE 'io wW 0020 <- 5a[0-9a-f]{6}' "$OUT/r2_$n.log" | tail -1)"
done
echo "===== wanda input silence (LS_TAG sends appear as ch0 tx in her log) ====="
echo "alice 0x77 hashes: $(grep -c 'io wW 0020 <- 77' "$OUT/r2_alice.log")"
echo "bob   0x77 hashes: $(grep -c 'io wW 0020 <- 77' "$OUT/r2_bob.log")"
echo "wanda 0x77 hashes: $(grep -c 'io wW 0020 <- 77' "$OUT/r2_wanda.log")"
echo "===== 3-way hash identity ====="
python3 /work/decode_det4.py "$OUT/r2_alice.log" "$OUT/r2_bob.log" "$OUT/r2_wanda.log"
echo SPEC3-DONE
